...
$owner = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM users WHERE id = {$ticket['user_id']}"));
...
if(isset($_POST['submit'])){
    ...
    if(!count($errs)){
        ...
        if(!mysqli_query($con, $query)){
            ...
            $konu = "Yeni Mesaj";
            $msg = "Destek talebinize yanıt verildi. Mesaj detayları aşağıdadır. Sisteme giriş yaparak inceleyebilirsiniz.";
            $content = str_replace('\r\n', '<br>', nl2br($content));
            $msg .= '<table>
                        <tr>
                            <td>Mesaj</td>
            ...
            mailgonder($owner['username'], $konu, $msg);
        }
    }
}
...
if(isset($_POST['s_submit'])){
    ...
    $query = "UPDATE tickets SET status = {$status} WHERE id = {$id} LIMIT 1";
        ...
        <form action="" method="post">
            <div class="form-group">
                ...
                <select name="status" id="status" class="form-control">
                    <option value="0" <?php echo $ticket['status'] == 0 ? 'selected' : ''; ?>>İncelenme Bekliyor</option>
                    <option value="1" <?php echo $ticket['status'] == 1 ? 'selected' : ''; ?>>İşlem Yapılıyor</option>
                    <option value="2" <?php echo $ticket['status'] == 2 ? 'selected' : ''; ?>>Cevaplandı</option>
                    <option value="3" <?php echo $ticket['status'] == 3 ? 'selected' : ''; ?>>Kapatıldı</option>
                </select>
            </div>
            <div class="form-group">
                <button class="btn btn-default form-control" name="s_submit" value="s_submit"><i class="fa fa-refresh"></i> Güncelle</button>
            </div>
        </form>
    </div>
</div>
